...
namespace NinjaTrader.NinjaScript.Strategies.Dotel
{
    public class Striker : Strategy
    {
        ...
        private double activeBreakoutHigh = double.NaN;
        private double activeBreakoutLow = double.NaN;
        private int activeHighBar = -1;
        private int activeLowBar = -1;
        ...
        protected override void OnBarUpdate()
        {
            ...
            bool breakLong = !cooldownActive && AllowLong && !double.IsNaN(activeBreakoutHigh) && CrossedAboveLevel(activeBreakoutHigh);
            bool breakShort = !cooldownActive && AllowShort && !double.IsNaN(activeBreakoutLow) && CrossedBelowLevel(activeBreakoutLow);
        ...
        private void UpdateCenteredBreakoutLevels()
        {
            int normalizedLookback = Lookback % 2 == 0 ? Lookback + 1 : Lookback;
            int centerBarsAgo = normalizedLookback / 2;

            if (CurrentBar < normalizedLookback)
                return;

            if (IsCenteredHigh(centerBarsAgo, normalizedLookback) && activeHighBar != CurrentBar - centerBarsAgo)
            {
                activeBreakoutHigh = High[centerBarsAgo];
                activeHighBar = CurrentBar - centerBarsAgo;
            }

            if (IsCenteredLow(centerBarsAgo, normalizedLookback) && activeLowBar != CurrentBar - centerBarsAgo)
            {
                activeBreakoutLow = Low[centerBarsAgo];
                activeLowBar = CurrentBar - centerBarsAgo;
            }
        }

        private bool IsCenteredHigh(int centerBarsAgo, int normalizedLookback)
        {
            double centerHigh = High[centerBarsAgo];
            int sideBars = normalizedLookback / 2;

            for (int offset = -sideBars; offset <= sideBars; offset++)
            {
                if (offset == 0)
                    continue;

                int barsAgo = centerBarsAgo + offset;
                if (barsAgo < 0 || High[barsAgo] >= centerHigh)
                    return false;
            }
        ...
        private bool IsCenteredLow(int centerBarsAgo, int normalizedLookback)
        {
            double centerLow = Low[centerBarsAgo];
            int sideBars = normalizedLookback / 2;
            ...
            for (int offset = -sideBars; offset <= sideBars; offset++)
            {
                ...
                int barsAgo = centerBarsAgo + offset;
                if (barsAgo < 0 || Low[barsAgo] <= centerLow)
                    return false;
            }
        ...
        {
            if (!ExportBacktestCsv || csvExported)
                return;
