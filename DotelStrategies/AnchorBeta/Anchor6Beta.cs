...
{
    public class Anchor6Beta : Strategy
    {
        // ====== Inputs (Usuario) ======
        ...
        public double CheckHourDecimal { get; set; } = 3.00;

        [NinjaScriptProperty]
        [Display(Name = "Hora ANCHOR (decimal, múltiplo 0.25)", GroupName = "02. Horas", Order = 1)]
        public double AnchorHourDecimal { get; set; } = 15.00;
        ...
        [Range(1, int.MaxValue)]
        public int TakeProfitTicks { get; set; } = 50;
        ...
        private static readonly DateTime ExpirationDate = new DateTime(2026, 4, 20);
        private const string AllowedMachineId = "64B1CDD3679276C4900736F898D11557";
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.Configure)
            {
                ...
                // IMPORTANTE:
                // Quitamos SetStopLoss / SetProfitTarget para que NinjaTrader
                // no vuelva a forzar los niveles al cerrar velas nuevas.
            }
            ...
            MarketPosition marketPosition, string orderId, DateTime time)
        {
            ...
            {
                // ====== NUEVO: crear SL y TP solo una vez al llenarse la entrada ======
                if (!exitsSubmitted && orderName == "AnchorLong")
                {
                    double avgFillPrice = execution.Order.AverageFillPrice;
                    int filledQty = execution.Order.Filled;

                    ExitLongStopMarket(0, true, filledQty, avgFillPrice - (StopLossTicks * TickSize), "AnchorLong_SL", "AnchorLong");
                    ExitLongLimit(0, true, filledQty, avgFillPrice + (TakeProfitTicks * TickSize), "AnchorLong_TP", "AnchorLong");

                    exitsSubmitted = true;
                    activeEntrySignal = "AnchorLong";
                }
                else if (!exitsSubmitted && orderName == "AnchorShort")
                {
                    double avgFillPrice = execution.Order.AverageFillPrice;
                    int filledQty = execution.Order.Filled;

                    ExitShortStopMarket(0, true, filledQty, avgFillPrice + (StopLossTicks * TickSize), "AnchorShort_SL", "AnchorShort");
                    ExitShortLimit(0, true, filledQty, avgFillPrice - (TakeProfitTicks * TickSize), "AnchorShort_TP", "AnchorShort");

                    exitsSubmitted = true;
                    activeEntrySignal = "AnchorShort";
                }

                if (orderName.IndexOf("Profit", StringComparison.OrdinalIgnoreCase) >= 0 || orderName.IndexOf("_TP", StringComparison.OrdinalIgnoreCase) >= 0)
                    tradeOutcomeText = "TP hit";
                else if (orderName.IndexOf("Stop", StringComparison.OrdinalIgnoreCase) >= 0 || orderName.IndexOf("_SL", StringComparison.OrdinalIgnoreCase) >= 0)
                    tradeOutcomeText = "SL hit";
                else if (marketPosition == MarketPosition.Flat && tradedToday)
                    tradeOutcomeText = "Position closed";
            }

            // Reset cuando ya no hay posición
