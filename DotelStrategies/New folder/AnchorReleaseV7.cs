...
{
    public class AnchorReleaseV7 : Strategy
    {
        private const int MaxSlots = 5;
        private const int DisabledMinutes = -1;
        private const double DisabledHourValue = -1.0;
        private const double KaizenBand1Multiplier = 1.28;
        private const double KaizenBand2Multiplier = 2.01;
        private const double KaizenBand3Multiplier = 2.51;

        private ScheduleSlot[] scheduleSlots;
        private TimeZoneInfo easternTimeZone;
        private KaizenVWAP kaizenVwap;
        ...
        private static readonly DateTime ExpirationDate = new DateTime(2026, 8, 20);
        private static readonly string[] AllowedMachineIds = new[]
        {
			"64B1CDD3679276C4900736F898D11557", //Mi machine ID
		    "79FDF03F1707DE25D5F6E1A1641706DB", // Angel Rojas LIFETIMEQ
...
//		    "C829A31CC9FBE4B2F4BA3F2CF80C9A77", // Rao Pago Mayo
        ...
        private string panelKaizenFilterText = "OFF";
        ...
        private TextBlock tbKaizenFilter;
        private Border sep2;
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                MinOpenGapTicks = 1;
                UseKaizenVwapFilter = false;
                UseKaizenDeviationFilter = false;
                KaizenDeviationBand = 1;
                AddKaizenVwapToChart = false;
            ...
            else if (State == State.Configure)
            {
                ...
                if (UseKaizenDeviationFilter && (KaizenDeviationBand < 1 || KaizenDeviationBand > 3))
                    throw new ArgumentOutOfRangeException(nameof(KaizenDeviationBand), "KaizenDeviationBand debe ser 1, 2 o 3.");
            ...
            else if (State == State.DataLoaded)
            {
                ...
                if (UseKaizenVwapFilter || UseKaizenDeviationFilter || AddKaizenVwapToChart)
                {
                    kaizenVwap = KaizenVWAP(KVWAPStyle.Directional, KaizenBand1Multiplier, KaizenBand2Multiplier, KaizenBand3Multiplier, 5, DashStyleHelper.Dash);
                    kaizenVwap.FillCloud = false;
                    kaizenVwap.ShowPriorVWAP = false;

                    if (AddKaizenVwapToChart && ShouldRenderVisuals())
                        AddChartIndicator(kaizenVwap);
                }
            }
        ...
        private bool TryExecuteSlot(ScheduleSlot slot, ref TradeDirection effectiveDirection)
        {
            ...
            if (!PassesKaizenFilters(signalDirection, checkPrice, out double kaizenVwapValue, out double kaizenBandPrice))
            {
                panelKaizenFilterText = BuildKaizenFilterStatusText(false, kaizenVwapValue, kaizenBandPrice);
                lastSignalText = string.Format("Slot {0}: filtro Kaizen no valido", slot.SlotNumber);
                RefreshStatusPanel();

                Print(string.Format(
                    "{0} | Slot {1} | Senal {2} omitida por filtro KaizenVWAP. Check={3} VWAP={4} BandaD{5}={6}.",
                    Time[0],
                    slot.SlotNumber,
                    signalDirection,
                    checkPrice,
                    double.IsNaN(kaizenVwapValue) ? "N/A" : kaizenVwapValue.ToString("F2"),
                    KaizenDeviationBand,
                    double.IsNaN(kaizenBandPrice) ? "N/A" : kaizenBandPrice.ToString("F2")));
                return false;
            }

            panelKaizenFilterText = BuildKaizenFilterStatusText(true, kaizenVwapValue, kaizenBandPrice);
        ...
        private bool PassesKaizenFilters(TradeDirection signalDirection, double price, out double vwap, out double bandPrice)
        {
            vwap = double.NaN;
            bandPrice = double.NaN;

            if (!UseKaizenVwapFilter && !UseKaizenDeviationFilter)
                return true;

            if (kaizenVwap == null)
                return false;

            vwap = kaizenVwap.CurrentVWAP;
            if (vwap <= 0)
                return false;
            ...
            if (signalDirection == TradeDirection.Long)
            {
                if (UseKaizenVwapFilter)
                    passesVwap = price < vwap;

                if (UseKaizenDeviationFilter)
                {
                    double stdDev = kaizenVwap.CurrentStdDev;
                    double multiplier = GetKaizenDeviationMultiplier();
        ...
        private string BuildKaizenFilterStatusText(bool passed, double vwap, double bandPrice)
        {
            if (!UseKaizenVwapFilter && !UseKaizenDeviationFilter)
                return "OFF";
            ...
            if (UseKaizenDeviationFilter)
            {
                ...
                return string.Format("{0} VWAP {1} / D{2} {3}", status, vwapText, KaizenDeviationBand, bandText);
            }

            return string.Format("{0} VWAP {1}", status, vwapText);
        }

        private double GetKaizenDeviationMultiplier()
        {
            if (KaizenDeviationBand == 1)
                return KaizenBand1Multiplier;

            if (KaizenDeviationBand == 2)
                return KaizenBand2Multiplier;

            if (KaizenDeviationBand == 3)
                return KaizenBand3Multiplier;
        ...
        private void ResetPanelTradeState()
        {
            ...
            panelDistanceTicks = double.NaN;
            panelMinDistanceMet = null;
            panelKaizenFilterText = UseKaizenVwapFilter || UseKaizenDeviationFilter ? "Pendiente" : "OFF";
            tradeOutcomeText = "Sin evaluar";
        ...
        private void TryCreatePanel()
        {
            ...
            ChartControl.Dispatcher.InvokeAsync(() =>
            {
                try
                {
                    ...
                    tbKaizenFilter = MakeTextBlock("", Brushes.Gold, FontWeights.Normal, PanelFontSize);
                    ...
                    statusPanelStack.Children.Add(tbKaizenFilter);
        ...
        private void RefreshStatusPanel()
        {
            ...
            ChartControl.Dispatcher.InvokeAsync(() =>
            {
                try
                {
                    ...
                    tbKaizenFilter.Text = string.Format("Kaizen: {0}", panelKaizenFilterText);
                    tbKaizenFilter.FontFamily = new FontFamily(PanelFontFamily);
                    tbKaizenFilter.FontSize = PanelFontSize;
                    tbKaizenFilter.Foreground = !UseKaizenVwapFilter && !UseKaizenDeviationFilter
                        ? Brushes.Gold
                        : (panelKaizenFilterText.StartsWith("Cumple", StringComparison.OrdinalIgnoreCase) ? Brushes.LimeGreen : Brushes.Red);
        ...
        [NinjaScriptProperty]
        [Display(Name = "Usar filtro Kaizen VWAP", GroupName = "03. Reglas", Order = 4)]
        public bool UseKaizenVwapFilter
        { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Usar filtro Kaizen desviacion", GroupName = "03. Reglas", Order = 5)]
        public bool UseKaizenDeviationFilter
        { get; set; }

        [NinjaScriptProperty]
        [Range(1, 3)]
        [Display(Name = "Desviacion Kaizen (1-3)", GroupName = "03. Reglas", Order = 6)]
        public int KaizenDeviationBand
        { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Mostrar KaizenVWAP", GroupName = "03. Reglas", Order = 7)]
        public bool AddKaizenVwapToChart
        { get; set; }
