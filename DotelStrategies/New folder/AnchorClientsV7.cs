...
namespace NinjaTrader.NinjaScript.Strategies.Dotel
{
    public class AnchorClientsV7 : Strategy
    {
        ...
        private static readonly DateTime ExpirationDate = new DateTime(2027, 1, 1);
        private static readonly string[] AllowedMachineIds = new[]
        {
		    ...
		    "4A49EAFF14A91054169B7D0B76E460D1", // Ergus LIFETIME
...
//		    "C829A31CC9FBE4B2F4BA3F2CF80C9A77", // Rao Pago Mayo
		    ...
		    //"6E5FF89447E468D19A2F3B6805D15ECD", // Gumbleberry 
		    //"A54CF05AA1743989C525B7D280B72FF7", // Armen 
        ...
        private string panelVwapFilterText = "OFF";
        ...
        private int lastBarScopedUpdateBar = -1;
        ...
        private TextBlock tbVwapFilter;
        private Border sep2;
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "Anchor V7 multi-slot calculado en cada tick, optimizado para backtest, con inversion de logica, breakeven, trailing positivo y trailing negativo de profit target.";
                Name = "AnchorClientsV7";
                ...
                MinDistanceTicks = 10;
                InvertSignalLogic = false;
                RequireDirectionalOpenGap = true;
                MinOpenGapTicks = 1;
                UseInternalVwapFilter = false;
                UseInternalVwapDeviationFilter = false;
                InternalVwapDeviationBand = 1;
                DrawInternalVwap = false;
                ...
                TrailingStepTicks = 2;
                UseNegativeTargetTrailing = false;
                NegativeTargetTrailingTriggerTicks = 6;
                NegativeTargetTrailingDistanceTicks = 8;
                NegativeTargetTrailingStepTicks = 2;
            ...
            else if (State == State.Configure)
            {
                ...
                if (UseNegativeTargetTrailing && NegativeTargetTrailingTriggerTicks <= 0)
                    throw new ArgumentOutOfRangeException(nameof(NegativeTargetTrailingTriggerTicks), "NegativeTargetTrailingTriggerTicks debe ser mayor que 0.");

                if (UseNegativeTargetTrailing && NegativeTargetTrailingDistanceTicks <= 0)
                    throw new ArgumentOutOfRangeException(nameof(NegativeTargetTrailingDistanceTicks), "NegativeTargetTrailingDistanceTicks debe ser mayor que 0.");

                if (UseNegativeTargetTrailing && NegativeTargetTrailingStepTicks <= 0)
                    throw new ArgumentOutOfRangeException(nameof(NegativeTargetTrailingStepTicks), "NegativeTargetTrailingStepTicks debe ser mayor que 0.");

                if (UseInternalVwapDeviationFilter && (InternalVwapDeviationBand < 1 || InternalVwapDeviationBand > 3))
                    throw new ArgumentOutOfRangeException(nameof(InternalVwapDeviationBand), "InternalVwapDeviationBand debe ser 1, 2 o 3.");
        ...
        protected override void OnBarUpdate()
        {
            ...
            bool enforceLicense = ShouldEnforceLicense();
            if (enforceLicense)
            {
                if (!IsMachineIdValid())
                {
            ...
            else
            {
                ...
                licenseStatusText = GetNonLiveLicenseStatus();
            }

            UpdateInternalVwap();
            ManageActiveTrades();

            if (lastBarScopedUpdateBar != CurrentBar)
            {
                lastBarScopedUpdateBar = CurrentBar;
                UpdateSessionGapState();
                DrawInternalVwapIfNeeded();
            ...
            MarketPosition marketPosition, string orderId, DateTime time)
        {
            ...
            if (execution.Order.OrderState == OrderState.Filled)
            {
                if (activeTrades.TryGetValue(orderName, out ActiveTradeState entryTrade))
                {
                    ...
                    entryTrade.CurrentTargetPrice = entryTrade.Direction == TradeDirection.Long
                        ? Instrument.MasterInstrument.RoundToTickSize(fillPrice + (TakeProfitTicks * TickSize))
                        : Instrument.MasterInstrument.RoundToTickSize(fillPrice - (TakeProfitTicks * TickSize));
                ...
                else
                {
                        ...
                        && activeTrades.TryGetValue(fromEntrySignal, out ActiveTradeState exitedTrade))
                    {
                        ...
                        if (orderName.IndexOf("Profit", StringComparison.OrdinalIgnoreCase) >= 0)
                            tradeOutcomeText = exitedTrade.NegativeTargetTrailingActivated
                                ? "TP trailing negativo"
                                : "TP alcanzado";
        ...
        private bool HasSlotTriggeredThisBar(ScheduleSlot slot)
        {
            ...
            DateTime currentEasternTime = GetEasternTime(Time[0]);
            int currentMinutes = ConvertTimeToMinutes(currentEasternTime);
            if (currentMinutes < slot.CheckMinutes)
                return false;

            if (CurrentBar == 0 || GetEasternTime(Time[1]).Date != currentEasternTime.Date)
                return true;

            int previousMinutes = ConvertTimeToMinutes(GetEasternTime(Time[1]));
        ...
        private bool TryExecuteSlot(ScheduleSlot slot, ref TradeDirection effectiveDirection)
        {
            ...
            if (!PassesInternalVwapFilters(signalDirection, checkPrice, out double filterVwapValue, out double filterBandPrice))
            {
                panelVwapFilterText = BuildInternalVwapFilterStatusText(false, filterVwapValue, filterBandPrice);
                lastSignalText = string.Format("Slot {0}: filtro VWAP no valido", slot.SlotNumber);
                RefreshStatusPanel();

                Print(string.Format(
                    "{0} | Slot {1} | Senal {2} omitida por filtro VWAP interno. Check={3} VWAP={4} BandaD{5}={6}.",
                    Time[0],
                    slot.SlotNumber,
                    signalDirection,
                    checkPrice,
                    double.IsNaN(filterVwapValue) ? "N/A" : filterVwapValue.ToString("F2"),
                    InternalVwapDeviationBand,
                    double.IsNaN(filterBandPrice) ? "N/A" : filterBandPrice.ToString("F2")));
                return false;
            }

            panelVwapFilterText = BuildInternalVwapFilterStatusText(true, filterVwapValue, filterBandPrice);
        ...
        private bool PassesInternalVwapFilters(TradeDirection signalDirection, double price, out double vwap, out double bandPrice)
        {
            vwap = double.NaN;
            bandPrice = double.NaN;

            if (!UseInternalVwapFilter && !UseInternalVwapDeviationFilter)
                return true;

            vwap = internalVwap;
            double stdDev = internalVwapStdDev;
            if (vwap <= 0 || sessionCumVolume <= 0)
                return false;

            bool passesVwap = true;
            bool passesDeviation = true;

            if (signalDirection == TradeDirection.Long)
            {
                if (UseInternalVwapFilter)
                    passesVwap = price < vwap;

                if (UseInternalVwapDeviationFilter)
                {
                    double multiplier = GetInternalVwapDeviationMultiplier();
                    if (stdDev <= 0 || multiplier <= 0)
                        return false;

                    bandPrice = vwap - (multiplier * stdDev);
                    passesDeviation = price < bandPrice;
                }

                return passesVwap && passesDeviation;
            }
        ...
        private string BuildInternalVwapFilterStatusText(bool passed, double vwap, double bandPrice)
        {
            if (!UseInternalVwapFilter && !UseInternalVwapDeviationFilter)
                return "OFF";

            string status = passed ? "Cumple" : "No cumple";
            string vwapText = double.IsNaN(vwap) ? "N/A" : FormatPrice(vwap);

            if (UseInternalVwapDeviationFilter)
            {
                string bandText = double.IsNaN(bandPrice) ? "N/A" : FormatPrice(bandPrice);
                return string.Format("{0} VWAP {1} / D{2} {3}", status, vwapText, InternalVwapDeviationBand, bandText);
            }

            return string.Format("{0} VWAP {1}", status, vwapText);
        }

        private double GetInternalVwapDeviationMultiplier()
        {
            if (InternalVwapDeviationBand == 1)
                return VwapBand1Multiplier;

            if (InternalVwapDeviationBand == 2)
                return VwapBand2Multiplier;

            if (InternalVwapDeviationBand == 3)
                return VwapBand3Multiplier;
        ...
        private void UpdateInternalVwap()
        {
            ...
            if (Bars.IsFirstBarOfSession && lastVwapBar != CurrentBar)
                ResetInternalVwap();

            double currentBarVolume = Volume[0];
            double deltaVolume = currentBarVolume;

            if (lastVwapBar == CurrentBar)
                deltaVolume = currentBarVolume - lastVwapBarVolume;
            else
            {
            ...
            if (deltaVolume <= 0)
            {
                lastVwapBarVolume = currentBarVolume;
                return;
            }

            double price = Close[0];
            sessionCumVolume += deltaVolume;
            sessionCumPriceVolume += price * deltaVolume;
            sessionCumPriceSquaredVolume += price * price * deltaVolume;
            lastVwapBarVolume = currentBarVolume;

            internalVwap = sessionCumVolume > 0 ? sessionCumPriceVolume / sessionCumVolume : 0;

            double variance = sessionCumVolume > 0
                ? (sessionCumPriceSquaredVolume / sessionCumVolume) - (internalVwap * internalVwap)
                : 0;

            internalVwapStdDev = variance > 0 ? Math.Sqrt(variance) : 0;
        }

        private void ResetInternalVwap()
        {
            internalVwap = 0;
            internalVwapStdDev = 0;
            priorInternalVwap = 0;
            sessionCumVolume = 0;
            sessionCumPriceVolume = 0;
            sessionCumPriceSquaredVolume = 0;
            lastVwapBar = -1;
            lastVwapBarVolume = 0;
        }

        private void DrawInternalVwapIfNeeded()
        {
            if (!DrawInternalVwap || !ShouldRenderVisuals() || ChartControl == null || CurrentBar < 1 || internalVwap <= 0 || priorInternalVwap <= 0)
                return;

            Draw.Line(this, "AnchorClientsV7_VWAP_" + CurrentBar, false,
                1, priorInternalVwap,
                0, internalVwap,
        ...
        private void ManageLongTrade(ActiveTradeState activeTrade)
        {
            ...
            if (double.IsNaN(activeTrade.WorstPrice) || Low[0] < activeTrade.WorstPrice)
                activeTrade.WorstPrice = Low[0];
            ...
            if (!double.IsNaN(desiredStopPrice)
                && (double.IsNaN(activeTrade.CurrentStopPrice) || desiredStopPrice > activeTrade.CurrentStopPrice))
            {
            ...
            ManageLongNegativeTargetTrailing(activeTrade);
        }
        ...
        private void ManageShortTrade(ActiveTradeState activeTrade)
        {
            ...
            if (double.IsNaN(activeTrade.WorstPrice) || High[0] > activeTrade.WorstPrice)
                activeTrade.WorstPrice = High[0];
            ...
            ManageShortNegativeTargetTrailing(activeTrade);
        }

        private void ManageLongNegativeTargetTrailing(ActiveTradeState activeTrade)
        {
            if (!UseNegativeTargetTrailing || double.IsNaN(activeTrade.WorstPrice))
                return;

            double adverseTicks = (activeTrade.EntryPrice - activeTrade.WorstPrice) / TickSize;
            if (adverseTicks < NegativeTargetTrailingTriggerTicks)
                return;

            double desiredTargetPrice = Instrument.MasterInstrument.RoundToTickSize(
                activeTrade.WorstPrice + (NegativeTargetTrailingDistanceTicks * TickSize));

            if (double.IsNaN(activeTrade.CurrentTargetPrice)
                || desiredTargetPrice < activeTrade.CurrentTargetPrice - (NegativeTargetTrailingStepTicks * TickSize))
            {
                activeTrade.CurrentTargetPrice = desiredTargetPrice;
                activeTrade.NegativeTargetTrailingActivated = true;
                SetProfitTarget(activeTrade.SignalName, CalculationMode.Price, desiredTargetPrice);
                lastSignalText = string.Format("{0}: TP negativo LONG activo", activeTrade.SignalName);
            }
        }

        private void ManageShortNegativeTargetTrailing(ActiveTradeState activeTrade)
        {
            if (!UseNegativeTargetTrailing || double.IsNaN(activeTrade.WorstPrice))
                return;

            double adverseTicks = (activeTrade.WorstPrice - activeTrade.EntryPrice) / TickSize;
            if (adverseTicks < NegativeTargetTrailingTriggerTicks)
                return;
                ...
                || desiredTargetPrice > activeTrade.CurrentTargetPrice + (NegativeTargetTrailingStepTicks * TickSize))
            {
                ...
                activeTrade.NegativeTargetTrailingActivated = true;
                SetProfitTarget(activeTrade.SignalName, CalculationMode.Price, desiredTargetPrice);
                lastSignalText = string.Format("{0}: TP negativo SHORT activo", activeTrade.SignalName);
            }
        }
        ...
        private TradeDirection GetSignalDirection(double anchorPrice, double checkPrice)
        {
            ...
            else if (anchorPrice < checkPrice)
                direction = TradeDirection.Short;

            if (!InvertSignalLogic || direction == TradeDirection.None)
                return direction;
        ...
        private string BuildEntrySignalName(ScheduleSlot slot, TradeDirection direction)
        {
            ...
            return string.Format("Anchor{0}_S{1}_{2}", directionName, slot.SlotNumber, ToDay(GetEasternTime(Time[0])));
        }

        private void ResetDailyStateIfNeeded()
        {
            DateTime barDate = GetEasternTime(Time[0]).Date;
        ...
        {
            if (!ShouldRenderVisuals() || ChartControl == null || scheduleSlots == null || scheduleSlots.Length == 0)
                return;
        ...
        {
            int dayKey = ToDay(GetEasternTime(referenceTime));
        ...
        {
            if (!ShouldRenderVisuals() || ChartControl == null)
                return;
        ...
        private void ResetPanelTradeState()
        {
            ...
            panelDistanceTicks = double.NaN;
            panelMinDistanceMet = null;
            panelVwapFilterText = UseInternalVwapFilter || UseInternalVwapDeviationFilter ? "Pendiente" : "OFF";
            tradeOutcomeText = "Sin evaluar";
        ...
        private void TryCreatePanel()
        {
            ...
            ChartControl.Dispatcher.InvokeAsync(() =>
            {
                try
                {
                    ...
                    tbVwapFilter = MakeTextBlock("", Brushes.Gold, FontWeights.Normal, PanelFontSize);
                    ...
                    statusPanelStack.Children.Add(tbVwapFilter);
                    ...
                    {
                        Name = "DotelAnchorClientsV7StatusPanel",
        ...
        private void RefreshStatusPanel()
        {
            ...
            ChartControl.Dispatcher.InvokeAsync(() =>
            {
                try
                {
                    ...
                    tbVwapFilter.Text = string.Format("VWAP: {0}", panelVwapFilterText);
                    tbVwapFilter.FontFamily = new FontFamily(PanelFontFamily);
                    tbVwapFilter.FontSize = PanelFontSize;
                    tbVwapFilter.Foreground = !UseInternalVwapFilter && !UseInternalVwapDeviationFilter
                        ? Brushes.Gold
                        : (panelVwapFilterText.StartsWith("Cumple", StringComparison.OrdinalIgnoreCase) ? Brushes.LimeGreen : Brushes.Red);
        ...
        {
            return !FastBacktestMode || State == State.Realtime;
        }
        ...
        private bool TryResolveAnchorPoint(DateTime checkBarTime, ScheduleSlot slot, out double resolvedAnchorPrice, out DateTime resolvedAnchorTime, out string error)
        {
            ...
            DateTime checkDay = GetEasternTime(checkBarTime).Date;
            ...
            int barsAgo = FindFirstBarAtOrAfterEastern(targetAnchorDateTime);

            if (barsAgo < 0)
            {
                error = string.Format("No hay bar disponible en o despues de {0:yyyy-MM-dd HH:mm:ss} EST.", targetAnchorDateTime);
                return false;
            }
        ...
        private bool TryGetPreviousDayWithData(DateTime referenceDay, out DateTime previousDayWithData, out string error)
        {
            ...
            for (int barsAgo = 0; barsAgo <= CurrentBar; barsAgo++)
            {
                DateTime barDate = GetEasternTime(Time[barsAgo]).Date;
        ...
        private int FindFirstBarAtOrAfterEastern(DateTime targetDateTime)
        {
            if (GetEasternTime(Time[0]) < targetDateTime)
                return -1;

            for (int barsAgo = 0; barsAgo <= CurrentBar; barsAgo++)
            {
                if (GetEasternTime(Time[barsAgo]) < targetDateTime)
                {
        ...
        private bool ShouldEnforceLicense()
        {
            ...
            try
            {
                ...
                if (accountName.IndexOf("Sim", StringComparison.OrdinalIgnoreCase) >= 0
                    || accountName.IndexOf("Playback", StringComparison.OrdinalIgnoreCase) >= 0
                    || accountName.IndexOf("Backtest", StringComparison.OrdinalIgnoreCase) >= 0)
                    return false;
            }
            catch
            {
        ...
        private string GetNonLiveLicenseStatus()
        {
            ...
            try
            {
                if (Account != null)
                {
                    ...
                    if (accountName.IndexOf("Playback", StringComparison.OrdinalIgnoreCase) >= 0)
                        return "PLAYBACK";

                    if (accountName.IndexOf("Backtest", StringComparison.OrdinalIgnoreCase) >= 0)
                        return "BACKTEST";
                }
            }
            catch
        ...
        private sealed class ActiveTradeState
        {
            ...
            public bool NegativeTargetTrailingActivated { get; set; }
        ...
        [NinjaScriptProperty]
        [Display(Name = "Invertir logica", GroupName = "03. Reglas", Order = 1)]
        public bool InvertSignalLogic
        { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Requerir gap apertura", GroupName = "03. Reglas", Order = 2)]
        public bool RequireDirectionalOpenGap
        { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "Gap minimo apertura (ticks)", GroupName = "03. Reglas", Order = 3)]
        public int MinOpenGapTicks
        { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Usar filtro VWAP interno", GroupName = "03. Reglas", Order = 4)]
        public bool UseInternalVwapFilter
        { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Usar filtro desviacion VWAP", GroupName = "03. Reglas", Order = 5)]
        public bool UseInternalVwapDeviationFilter
        { get; set; }

        [NinjaScriptProperty]
        [Range(1, 3)]
        [Display(Name = "Desviacion VWAP (1-3)", GroupName = "03. Reglas", Order = 6)]
        public int InternalVwapDeviationBand
        { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Dibujar VWAP interno", GroupName = "03. Reglas", Order = 7)]
        public bool DrawInternalVwap
        { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Modo rapido backtest", GroupName = "06. Backtest", Order = 0)]
        public bool FastBacktestMode
        { get; set; }
        ...
        [NinjaScriptProperty]
        [Display(Name = "Usar trailing negativo TP", GroupName = "04. Riesgo", Order = 9)]
        public bool UseNegativeTargetTrailing
        { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "TP negativo trigger (ticks)", GroupName = "04. Riesgo", Order = 10)]
        public int NegativeTargetTrailingTriggerTicks
        { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "TP negativo distancia (ticks)", GroupName = "04. Riesgo", Order = 11)]
        public int NegativeTargetTrailingDistanceTicks
        { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "TP negativo step (ticks)", GroupName = "04. Riesgo", Order = 12)]
        public int NegativeTargetTrailingStepTicks
        { get; set; }
    }
}
