...
{
    public class AnchorPricev6SundayTest : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "Prueba de domingo: valida operativa en la primera hora del domingo usando el ultimo dia con datos previo como anchor.";
                Name = "AnchorPricev6SundayTest";
        ...
        private bool TryResolveAnchorPoint(DateTime checkBarTime, ScheduleSlot slot, out double resolvedAnchorPrice, out DateTime resolvedAnchorTime, out string error)
        {
            ...
            if (checkBarTime.DayOfWeek == DayOfWeek.Sunday)
            {
                Print(string.Format(
                    "SUNDAY TEST | Slot {0} | Check={1:yyyy-MM-dd HH:mm:ss} | AnchorDay={2:yyyy-MM-dd} | Target={3:yyyy-MM-dd HH:mm:ss} | Resolved={4:yyyy-MM-dd HH:mm:ss} | Price={5}",
                    slot.SlotNumber,
                    checkBarTime,
                    anchorDay,
                    targetAnchorDateTime,
                    resolvedAnchorTime,
                    resolvedAnchorPrice));
            }
